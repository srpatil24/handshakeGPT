...
def save_tcp_packets_from_pcap(input_file):
    ...
    # Create a list to store packet dictionaries
    packet_list = []

    # Convert packets to dictionaries and store in the list
    for packet in tqdm(packets, desc="Processing packets", unit="packet"):
        if packet.haslayer(TCP) and packet.haslayer(IP):
            packet_dict = packet_to_dict(packet)
            packet_list.append(packet_dict)

    # Save the packet list as a JSON file
    output_file = os.path.join(output_folder, 'tcp_packets_2.json')
    with open(output_file, 'w') as f:
        json.dump(packet_list, f, indent=2)

# Call the function with the input pcap file name
save_tcp_packets_from_pcap('Pcap Files/bigFlows.pcap')
