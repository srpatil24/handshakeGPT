...
def save_tcp_streams_from_pcap(input_file, output_file):
    # Read the pcap file
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Save TCP streams with three-way handshakes from a pcap file.')
    parser.add_argument('--input', required=True, help='Path to the input pcap file.')
    parser.add_argument('--output', default='handshakes.json', help='Path to the output JSON file. Default: handshakes.json')

    args = parser.parse_args()

    # Call the function with the input pcap file name and output JSON file name
    save_tcp_streams_from_pcap(args.input, args.output)
